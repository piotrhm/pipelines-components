...
1. **Data loading & splitting** (``timeseries_data_loader``): Loads CSV from S3 (up to 100 MB).
   Replaces ``±inf`` with NaN; keeps missing targets for AutoGluon’s model-specific NaN handling.
   Requires parseable timestamps and non-null ids (invalid timestamps raise; rows are not silently
   dropped to preserve regular frequency). Deduplicates ``(id, timestamp)`` (keep last).
   Then applies a two-stage **per-series temporal** split on ``id_column`` / ``timestamp_column``:
