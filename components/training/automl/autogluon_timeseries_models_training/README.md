...
Writes ``component_status.json`` under ``component_status`` with ``component_id`` ``autogluon_timeseries_models_training`` and training stages (``load_data``, ``model_selection`` with steps ``feature_engineering``, ``model_training``, ``stacking``, ``model_evaluation``, ``refit_and_evaluate``). Artifact metadata display name: **Timeseries Models Training Status**.
