...
Reads the documents_descriptor JSON (from documents_discovery), fetches the listed documents from S3, and extracts text using the docling library. Results are persisted as DoclingDocument JSON files (one per input document).
